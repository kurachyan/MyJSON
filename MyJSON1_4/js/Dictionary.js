...
(function (window) {
	...
	let dictionaryID = null;         // ディクショナリ専用ＩＤ
	let dictionaryFlag = new Boolean(false);     // 辞書運用
	let Table = new Array();
	let pos;
	...
	function LocalDictionaryScript() {
		...
		// モジュール名：Get
		// 　　　　入力：eno　・・・　取り出す番号
		// 　　　　出力：Table[eno].DLink　・・・　抜き取り情報
		// 　　処理内容：資源テーブル情報を取り出す
		this.Get = function (eno) {
			let ret = null;                 // 未定義

			if (eno < this.Getpos()) {      // 取り出し位置が資源テーブル内か？
				let ctype = Table[eno].kind;

				if ((ctype == 'M') || (ctype == 'V')) {     // 必要な情報か？
					ret = Table[eno].DLink;
				}
			}
...
(function (window) {
	...
	function SubDictionaryScript() {
		...
		this.Add = function (wnumber, wlink, wname, wlogic) {
			let o3 = this.Init();
			o3.Number = wnumber;
			o3.Link = wlink;
			o3.Name = wname;
		...
		this.vAdd = function (wname, wargc, wargv) {
			let o5 = this.vInit();
			o5.Name = wname;
			o5.Argc = wargc;
			o5.Argv = wargv;
